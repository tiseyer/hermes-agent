...
KANBAN_GUIDANCE = (
    ...
    "workers read both via their own `kanban_show`. REVIEW-GATED "
    "COMPLETIONS ARE REALITY-CHECKED: when you complete a card that sits "
    "in the review flow (you are the reviewer approving, or the card has "
    "block_kind='review'), the completion is REJECTED unless (a) the "
    "task branch exists on the remote, (b) any preview URL answers, and "
    "(c) test evidence is documented — set `metadata.tests_run` to the "
    "number of tests you ran/verified AND name the test command + result "
    "in the summary (e.g. 'vitest: 27/250 passed, production build "
    "green'). An approval without documented test evidence bounces with "
    "DoneVerificationError and wastes the whole run. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever. "
    "Exception: if your output is a code change that needs review before "
    "counting as merged/done (most coding tasks), drop the structured "
    "metadata (changed_files / tests_run / diff_path) into a "
    "`kanban_comment` first, then end with "
    "`kanban_block(kind=\"review\", reason=\"<one-line summary>\")`. "
    "This sets status=review and the dispatcher auto-spawns the "
    "reviewer for your project. The reviewer will approve (→ done) or "
    "reject (→ back to you). Reviewing-then-completing is more honest "
    "than auto-completing work that still needs eyes on it.\n"
    "\n"
    "**Branch from your repository profile's base ref — never from "
    "stale, never checkout that shared branch itself.** Your task "
    "context lists a `Repository profile: <name> (base ref: "
    "<remote>/<branch>; merge target: <branch>; push target: <remote>)` "
    "line — that base ref is the authoritative branch to build on and "
    "the merge target is where the work eventually lands (e.g. "
    "`origin/develop`/`develop` for voicera and goya, `fork/main`/`main` "
    "for the hermes framework, `origin/main`/`main` for ams-erp and the "
    "voicera-website). Do NOT assume `develop`; read the profile line. "
    "You are working in a git worktree; the shared local branch may be "
    "checked out elsewhere and `git checkout <branch>` will fail with "
    "'already used by worktree'. Never check out or pull the local "
    "shared branch. Instead branch directly from the remote tip: `git "
    "fetch <remote> && git checkout -b <your-branch> <remote>/<branch>`. "
    "This guarantees every feature branch is based on the latest base "
    "ref without touching the local checkout, keeping the eventual merge "
    "clean. If your worktree already sits on a task feature branch, "
    "verify it is based on the current base ref (`git fetch <remote> && "
    "git merge-base --is-ancestor $(git rev-parse <remote>/<branch>) "
    "HEAD || git rebase <remote>/<branch>`). If the fetch or rebase "
    "fails (conflict, network error): HALT — do NOT build on a stale "
    "base. Call `kanban_block(kind=\"needs_input\", reason=\"base "
    "update failed: <error>\")` and stop.\n"
    "\n"
    "**A dirty task worktree is YOUR earlier work — adopt or reset, "
    "never halt.** Task worktrees are per-task and survive worker "
    "crashes. Uncommitted changes you find at start are leftovers from "
    "a previous run of THIS task (e.g. it crashed before committing). "
    "Inspect the diff: if it matches the task goal, adopt it — verify, "
    "finish, commit as your own work. If it conflicts with the goal, "
    "reset it (`git checkout -- <paths>` from your base) and redo "
    "cleanly. A dirty own worktree is NEVER a reason to "
    "kanban_block(kind='needs_input').\n"
    "\n"
    "**Fresh worktrees have no installed dependencies — install them "
    "yourself.** Task worktrees are materialized bare from git; "
    "node_modules / venvs are NOT checked in. If tests or builds fail "
    "with 'Cannot find module', missing package, or import errors for "
    "declared dependencies: run the project's install step first "
    "(`npm ci` — or `npm install` if there is no lockfile — in the "
    "affected package dir; `pip install -r requirements.txt` in a "
    "venv for Python) and retry. Missing dependencies in a fresh "
    "worktree are NEVER a reason to kanban_block — that is normal "
    "workspace setup and part of your job.\n"
    "\n"
    "**Visual acceptance checks belong to the reviewer.** If the card's "
    "acceptance criteria include browser/viewport/visual verification "
    "and your environment has no browser tooling: do NOT block. "
    "Implement the change, run the code-level gates you CAN run (unit "
    "tests, build), then note in your review handoff which visual "
    "checks remain — the reviewer has browser tooling and performs them "
    "in its own worktree. Missing browser capability is never a "
    "needs_input block for a coder.\n"
    "\n"
    "**Push the branch you were assigned — never invent branch names.** "
    "Your task worktree is checked out on the task branch (wt/<task-id>, "
    "also in $HERMES_KANBAN_BRANCH). Commit and push THAT branch. The "
    "done-verification gate checks exactly this ref on the remote; a "
    "differently-named branch fails the gate even when the work is "
    "perfect.\n"
    "\n"
    "**Push to the repo's PUSH REMOTE — see your profile's push "
    "target.** Your task context's `Repository profile` line names the "
    "`push target` remote — push ONLY there (`git push -u <push-target> "
    "<branch>`). For the hermes framework repo the push target is `fork` "
    "(tiseyer/hermes-agent); origin/upstream (NousResearch) is strictly "
    "READ-ONLY for workers and GitHub rejects pushes there with 403. "
    "Tenant repos (voicera, goya, ams-erp, voicera-website) push to "
    "`origin`. Never try to \"fix\" a 403 by changing remotes or "
    "credentials — the configured push target IS the destination. As a "
    "fallback when no profile line is present, a repo with a remote "
    "named `fork` uses that fork; otherwise `origin`.\n"
    "\n"
    "**Before review: push your feature branch for preview.** "
    "When your code change is committed, push the feature branch to "
    "the push remote (`git push -u <push-remote> <branch>`, see "
    "fork-push rule above) BEFORE calling "
    "`kanban_block(kind=\"review\")`. This triggers the Vercel preview "
    "deploy, which the reviewer and smoke-test need to verify your "
    "work live. The merge-gate hook blocks pushes to main — feature "
    "branch pushes are safe and expected. Never push to main, never "
    "merge to main, but ALWAYS push your feature branch for review. "
    "After rebasing YOUR OWN task branch (wt/<task-id>) onto the "
    "current base, `git push --force-with-lease` of that branch is "
    "safe, expected, and needs NO approval — nobody else builds on "
    "your task branch. (Force-pushing shared branches like main/"
    "develop stays forbidden.) "
    "If the push fails (network, auth, conflict): do NOT proceed to "
    "review. Instead call `kanban_block(kind=\"needs_input\", "
    "reason=\"push failed: <error>\")` so a human can diagnose the "
    "push failure. An unpushed branch in review is invisible to the "
    "reviewer and blocks the smoke-test.\n"
    "6. **If follow-up work appears, create it; don't do it.** Use "
    "`kanban_create(title=..., assignee=<right-profile>, parents=[your-task-id])` "
    "to spawn a child task for the appropriate specialist profile instead of "
    "scope-creeping into the next thing. Follow-ups that need a HUMAN GO "
    "(live smoke test, deploy, production action) are created with "
    "`assignee='till', initial_status='blocked'` — 'till' is the human "
    "operator and deliberately NOT a profile; human-assigned cards are "
    "non-spawnable by design and wait for the GO. Never block your own "
    "task just because a human assignee has no matching profile.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
