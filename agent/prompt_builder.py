...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Branch from current origin/develop — never from stale, never "
    "checkout develop itself.** You are working in a git worktree; the "
    "local `develop` branch is checked out elsewhere and `git checkout "
    "develop` will fail with 'already used by worktree'. Never check out "
    "or pull local `develop`. Instead branch directly from the remote "
    "tip: `git fetch origin && git checkout -b <branch> origin/develop`. "
    "This guarantees every feature branch is based on the latest develop "
    "without touching the local develop checkout, keeping the eventual "
    "merge clean. If your worktree already sits on a task feature "
    "branch, verify it is based on current origin/develop (`git fetch "
    "origin && git merge-base --is-ancestor $(git rev-parse "
    "origin/develop) HEAD || git rebase origin/develop`). If the fetch "
    "or rebase fails (conflict, network error): HALT — do NOT build on "
    "a stale base. Call `kanban_block(kind=\"needs_input\", "
    "reason=\"base update failed: <error>\")` and stop.\n"
    "\n"
    ...
    "perfect.\n"
    "\n"
    "**Push to the repo's PUSH REMOTE — fork-push rule.** If the repo "
    "has a remote named `fork`, that fork is the ONLY remote you may "
    "push to (`git push -u fork <branch>`); origin/upstream is strictly "
    "READ-ONLY for workers and GitHub rejects pushes there with 403 "
    "(this framework repo: upstream is NousResearch/hermes-agent, the "
    "writable fork is tiseyer/hermes-agent). Never try to \"fix\" a 403 "
    "by changing remotes or credentials — the fork IS the destination. "
    "Repos without a `fork` remote keep `origin` as push target.\n"
    "\n"
