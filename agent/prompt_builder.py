...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "**Branch from current develop — never from stale.** "
    "Before you create a feature branch, always update develop first: "
    "`git fetch origin && git checkout develop && git pull`. Then branch "
    "from the tip: `git checkout -b <branch>`. This guarantees every "
    "feature branch is based on the latest develop, keeping the eventual "
    "merge clean. A branch created from a stale develop (behind by "
    "commits) causes rebase conflicts that are avoidable at branch time "
    "but expensive at merge time. If the develop update fails "
    "(uncommitted changes in the way, merge conflict, network error): "
    "HALT — do NOT build on a stale base. Call `kanban_block("
    "kind=\"needs_input\", reason=\"develop update failed: <error>\")` "
    "and stop.\n"
    "\n"
